...
/**
 * Send a read-only JSONEx (JSON Extended) request to TallyPrime
 * @param {object} params
 ...
 */
async function sendJsonExRequest({ request, host = env.tally.host, port = env.tally.port, timeoutMs = env.tally.timeoutMs }) {
  ...
  try {
    ...
    if (!isJson || hasTallyError) {
      const errMsg = hasTallyError
        ? (Array.isArray(parsedJson.error_list) ? parsedJson.error_list.join("; ") : (parsedJson.error || "Tally returned JSONEx error"))
        : "Tally environment returned non-JSON data when JSONEx export was requested.";
      ...
      return {
        ...
        errorCode: "TALLY_JSONEX_UNSUPPORTED",
        errorMessage: errMsg
      };
    }
    ...
    const responseTimeMs = Date.now() - startedAt;
    logger.debug({ url, error: error.message, code: error.code }, "JSONEx transport error");
